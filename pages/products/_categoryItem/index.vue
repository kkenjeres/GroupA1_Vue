    ...
    <div class="bx--grid">
      <section class="product-header">
        <div class="bx--row">
          <div class="bx--col-md-13 bx--col-lg-13">
            <h3>
              {{ breadcrumb }}
              <span v-if="$route.query.manufactur !== undefined">{{
                $route.query.manufactur
              }}</span>
            </h3>
          ...
          <div class="bx--col-md-3 bx--col-lg-3">
            <cv-dropdown placeholder="Производитель">
              <nuxt-link
                v-for="tag in tags.data.tags"
                :key="tag.name"
                :to="{
                  path: $route.params.path,
                  query: { manufactur: tag.name },
                }"
              >
      ...
      <div class="bx--row">
        <product-preview
          ...
          :slug="product.slug.replace('products/', '')"
...
export default {
  ...
  // загрузка тегов в фильтрацию через $store
  async fetch  ({ store, params }) {
    await store.dispatch('productFilter/loadProductsTags', { tagsPoint: 'products/' + params.categoryItem + '/' })
  },

  // загрузка всех продуктов категории через $store
  async asyncData ({ store, params, query, error }) {
    try {
      await store.dispatch('productFilter/loadAllProducts', { startWith: 'products/' + params.categoryItem + '/', withTag: query.manufactur, perOnePage: query.per, pageNumber: query.page })
    } catch (er) {
  ...
  computed: {
    ...
    breadcrumb () {
      ...
      for (const key in this.$store.state.sitemap) {
        if (this.$store.state.sitemap[key].item.real_path === this.$route.path) {
          item = this.$store.state.sitemap[key].item.name
        }
      }
      return item
    }
